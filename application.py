...
def getAllPost():
    if request.method == "GET":
        try:
            ...
            return json.dumps([{"id": data[0], "date_time": str(data[1]), "post_content": data[2], "post_status":data[3], "post_type": data[4], "post_like_count":data[5], "post_title": data[7]} for data in result])
        finally:
            cursor.close()
...
def changeStatus():
    try:
        ...
        status = request.get_json()["post_status"]
        cursor.execute("""
            UPDATE  posts 
            SET post_status = %s
            WHERE post_ID = %s
